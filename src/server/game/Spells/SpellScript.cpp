...
int32 SpellScript::GetHitAbsorb()
{
    if (!IsInTargetHook())
    {
        LOG_ERROR("spells.scripts", "Script: `{}` Spell: `{}`: function SpellScript::GetHitAbsorb was called, but function has no effect in current hook!", m_scriptName->c_str(), m_scriptSpellId);
        return 0;
    }
